...
if (isset($_GET['dbKey'])) {
    $db_name = $database_folder.'/'.$_GET['dbKey'].'.db';
...
if (isset($_GET['addColumn'])) {
    ...
    $stmt = $db->prepare("INSERT INTO columns (column_name, color, position) VALUES (:column_name, :color, :position)");
    $stmt->bindParam(':position', $data['position']);
    $stmt->bindParam(':color', $data['color']);
    $stmt->bindParam(':column_name', $data['column_name']);
    $stmt->execute();

    $column_id = $db->lastInsertId();

    response(array('message' => 'success', 'column_id' => $column_id));
}
...
if (isset($_GET['editColumn'])) {
    ...
    $stmt = $db->prepare("UPDATE columns SET column_name = :column_name, color = :color WHERE id = :id");
    $stmt->bindParam(':color', $data['color']);
    $stmt->bindParam(':column_name', $data['column_name']);
