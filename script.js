...
document.addEventListener(
    ...
    (ev) => {
        ...
        ev.target.classList.add('topic--dragging');
    },
    false
);
document.addEventListener(
...
document.addEventListener(
    ...
    (ev) => {
        ...
        if (ev.target.classList.contains('column__topics')) {
            ...
            const draggedTopicId = ev.dataTransfer.getData('text/plain');
            const draggedTopic = document.getElementById(draggedTopicId);
            ev.target.appendChild(draggedTopic);

            const columnId = ev.target.parentElement.id.split('_')[1];
            const topicId = draggedTopicId.split('_')[1];
              ...
              `;
                ...
                if (topics[column.id]) {
                    topics[column.id].forEach((topic) => {
                      ...
                      <span class="topic_edit" onclick="showEditTopicForm(event)">E</span>
                        <span class="topic_delete" onclick="deleteTopic(event)">D</span>
...
function showNewTopicForm(event) {
    ...
    newTopic.innerHTML = `
        <header class="new-topic__header">
            <h2>New Topic</h2>
            <span class="new-topic__header__close header_icon icon_close"  onclick="closeNewTopic(event)"></span>
        </header>
